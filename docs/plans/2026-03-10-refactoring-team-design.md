...
| 角色 | 名称 | Provider | 核心职责 |
|------|------|----------|----------|
| **策划 Lead** | `lead` | Claude | 模块影响分析、迁移计划制定、方案审批、进度跟踪、不写代码 |
| **开发 A** | `dev-admin` | Codex | admin app 模块迁移、基础设施搭建 |
| **开发 B** | `dev-platform` | Codex | platform app 模块迁移、公共组件/工具抽取到 packages/ |
...
**策划 Lead：**
- 不参与代码开发
- 每次迁移前：分析原项目模块依赖 → 评估目标项目影响范围 → 输出迁移方案
- 追踪 `doc/task/task.md` 中的任务进度
- 审批开发方案

**开发 A/B：**
- 每次开发前必须确认改动影响范围（GitNexus impact analysis）
...
| 检查点 | 负责人 | 内容 |
|--------|--------|------|
| **开发前** | 策划 Lead | 原项目依赖分析 + ai_monorepo 影响评估 + 方案审批 |
| **开发中** | 开发 A/B | GitNexus impact analysis 确认改动范围 |
