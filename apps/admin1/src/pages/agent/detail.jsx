...
import { useChangeDetection, getDetailComponentStateKeys } from "./utils/changeDetection"
import "./detail.scss"
...
const AgentDetail = () => {
  ...
  // 使用变更检测 Hook
  const hasChanges = useChangeDetection(agentDetail, currentStates, getDetailComponentStateKeys())

  // 检测未保存的更改
  useEffect(() => {
    if (agentDetail) {
      // 使用全面的变更检测
      const hasChangesDetected = hasChanges()
      setHasUnsavedChanges(hasChangesDetected)

      // 调试日志：输出检测结果
      console.log("变更检测结果:", {
        hasChangesDetected,
        agentDetailPrompt: agentDetail.prompt,
        markdownContent,
        agentDetailPluginNos: agentDetail.pluginNos,
        selectedTools
      })
    }
  }, [agentDetail, currentStates, hasChanges])
    ...
    async (
      ...
      try {
        ...
        if (response.success) {
          ...
          // 保存成功后立即更新 agentDetail 的版本号，确保变更检测能正确工作
          // 这样可以避免由于 agentDetail 没有及时更新导致的检测错误
          if (agentDetail) {
            setAgentDetail((prev) => ({
              ...prev,
              versionNo: response.data?.versionNo || prev.versionNo,
              prompt: content || prev.prompt,
              pluginNos: pluginNos || selectedTools,
              skillNos: skillNos || selectedSkills,
              knowledgeBases: newKnowledgeBases || knowledgeBases,
              model: model || selectedModel,
              type: convertedType,
              workSkillNos: workSkillNos || prev.workSkillNos
            }))
          }

          // 保存成功后重新获取最新数据，确保变更检测能正确工作
          // 使用 setTimeout 避免在保存回调中直接调用异步函数
          // setTimeout(() => {
          //   fetchLatestVersion()
          // }, 0)
        } else {
