...
import { QuestionCircleOutlined, SyncOutlined, InfoCircleOutlined } from "@ant-design/icons"
...
import "./index.less"
import CallDashboard from "./components/CallDashboard/index"
...
//       />
//     ),
//     [modelBillingData, adjustedStartTime, adjustedEndTime]
//   )
...
function DailyReport() {
  ...
  console.log("props", botNo, iframeStyle)
  ...
  const { za_admin_integration } = searchParams
  console.log("searchParams", isIframe, searchParams, hashParams)
  ...
  const searchData = useCallback(
    (searchValues) => {
      ...
      if (isIframe || za_admin_integration) {
        return
      }

      Promise.all([
        // 调用次数和人数数字面板
        fetchCallLogsOverview(searchValues),

        // 趋势大盘
        fetchMetricCyclesList(searchValues),

        // 饼状图1
        fetchPieChartOfPerSkillUsage({ ...searchValues }),

        // 饼状图2
        fetchSkillOverview({ ...searchValues, proportionCode: "skillTypeCallPieChart" }),

        // TOP10空间柱状图
        fetchSkillOverview({ ...searchValues, size: 10, proportionCode: "countRobotCalls" }),

        // Top10工作流柱状图
        fetchSkillOverview({ ...searchValues, size: 10, proportionCode: "countSkillCalls" }),

        // 请求人Top20柱状图
        fetchSkillOverview({ ...searchValues, size: 20, proportionCode: "countCallUsers" }),

        // 获取模型计费数据
        // 使用工具函数调整查询时间范围（T+1限制 + 最多7天限制）
        (() => {
          // 如果没有 botNo，则不调用计费接口
          if (!botNo) {
          ...
          return {
            modelBillingData: response?.modelBillingData || {},
            totalModelBilling: response?.totalModelBilling || 0
          }
        })
      ])
        .then(
          ([
            ...
            stableDispatch.setNumberList([
              {
                title: "调用次数",
                value: callLogsResult?.totalSkillCalls || 0
              },
              {
                title: "调用人数",
                value: callLogsResult?.totalCallUsers || 0
              },
              {
                title: "模型计费总额(元)",
                value: modelBillingResult?.totalModelBilling || 0
              }
            ])

            // 设置趋势大盘
            if (Array.isArray(metricCyclesResult)) {
              stableDispatch.setSegmentedOptions(metricCyclesResult)
              const firstAvailableItem = metricCyclesResult.find(
                (item) => item.disabled === false && item.value
              )
            ...
            // 设置饼状图1
            stableDispatch.setPieData1(pie1Result?.items || [])

            // 设置饼状图2
            stableDispatch.setPieData2(pie2Result?.items || [])

            // 设置TOP10空间柱状图
            stableDispatch.setTopRobots(topRobotsResult || [])

            // 设置Top10工作流柱状图
            stableDispatch.setTopSkills(topSkillsResult || [])

            // 设置请求人Top20柱状图
            stableDispatch.setTopUsers(topUsersResult || [])

            // 设置模型计费数据
            stableDispatch.setModelBillingData(modelBillingResult?.modelBillingData || {})
            stableDispatch.setTotalModelBilling(modelBillingResult?.totalModelBilling || 0)

            // 设置调整后的时间范围
            const { adjustedStartTime, adjustedEndTime } = adjustQueryDateRange(
              searchValues.startTime,
              searchValues.endTime
            )
  ...
  return (
    <div
      className={`bg-[#EFF1F4] max-h-[100vh]  report-forms ${!isIframe || !za_admin_integration ? "" : "overflow-auto"}`}
    >
      <div className="p-5 bg-white rounded-md h-auto">
        {isIframe && !za_admin_integration && (
        ...
        {activeTab === "call" ? (
          <div
            className="pb-[20px]"
            style={za_admin_integration ? { marginTop: "50px" } : { marginTop: "30px" }}
          >
            {!isIframe || za_admin_integration ? (
              <CallDashboard
                key={refreshKey}
                ...
                topSkills={topSkills}
                topUsers={topUsers}
                modelBillingData={modelBillingData}
