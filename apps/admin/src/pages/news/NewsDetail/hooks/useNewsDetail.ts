...
import { NewsDetailAPI, NewsEditAPI, NewsAPI } from '@/api/news'
import type { ColumnsType, TablePaginationConfig } from 'antd/es/table'
import type { Key } from 'react'
import type { NewsItem, FetchDataParams } from '../types'
...
export const useNewsDetail = () => {
  ...
  const fetchTypeList = async () => {
    try {
      // Reserved for future category list fetching
      // setTypeList will be called here once the API is available
      void setTypeList
    } catch (error) {
  ...
  const fetchData = async (params: FetchDataParams = {}) => {
    ...
    try {
      ...
      const response = (await NewsDetailAPI.getList(queryParams)) as unknown as ApiResponse<{
        data: NewsItem[]
        totalCount: number
        pageNum: number
      }>
  ...
  // 操作处理函数
  const handleView = (record: NewsItem) => {
  ...
  const handleDelete = async (newsNo: string) => {
    try {
      const response = (await NewsAPI.delete(newsNo)) as unknown as ApiResponse
      if (response?.success) {
  ...
  const handleBatchDelete = async () => {
    ...
    Modal.confirm({
      ...
      onOk: async () => {
        try {
          const response = (await NewsAPI.batchDelete(selectedRowKeys as string[])) as unknown as ApiResponse
          if (response.success) {
  ...
  // 工具函数
  const getStatusTag = (status: string): ReactNode => {
    const config = statusMap[status] || statusMap.wait
    return React.createElement(Tag, { color: config.color, className: config.className }, config.text)
  }

  const getTypeName = (typeId: any): string => {
    const type = typeList.find((item: any) => item.id === typeId)
    return type ? type.name : '未知类别'
  }

  const handleSwitchChange = async (checked: boolean, record: NewsItem) => {
    setLoading(true)
    try {
      const data = {
        title: record?.title,
      ...
      const response = (await NewsEditAPI.publish(data as any)) as unknown as ApiResponse
      if (response?.success) {
        fetchData()
      } else {
        message.error(response?.message)
    ...
    () => [
      ...
      },
      {
        ...
        render: (categoryName: string) =>
          React.createElement(Tag, { color: 'blue', style: { margin: 0 } }, categoryName || '未分类'),
      },
      {
        title: '封面图',
        dataIndex: 'cover',
      ...
      },
      {
        ...
        render: (summary: string) =>
          React.createElement(
            Tooltip,
            { placement: 'topLeft', title: summary },
            React.createElement(
              'div',
              {
                style: {
      ...
      },
      {
        ...
        key: 'publishTime',
        width: 160,
        render: (publishTime: string | null) =>
          publishTime
            ? React.createElement('div', { style: { fontSize: '13px', color: '#666' } }, publishTime)
            : React.createElement(
                'span',
      ...
      },
      {
        ...
        key: 'creator',
        width: 160,
        render: (creator: string) => React.createElement('div', {}, creator),
      },
      {
        title: '创建时间',
        ...
        render: (gmtCreated: string) =>
          React.createElement('div', { style: { fontSize: '13px', color: '#666' } }, gmtCreated),
      },
      {
        title: '更新时间',
        ...
        render: (gmtModified: string) =>
          React.createElement('div', { style: { fontSize: '13px', color: '#666' } }, gmtModified),
      },
      {
        title: '操作',
        key: 'action',
  ...
  const tabItems = useMemo(() => {
    ...
    const typeItems = typeList.map((type: any) => {
      const typeCount = dataSource.filter((item: NewsItem) => (item as any).categoryId === type.id).length
      return {
