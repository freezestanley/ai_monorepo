...
const SkillsAndKnowledge = forwardRef(
  (
    ...
    const [processPopoverVisible, setProcessPopoverVisible] = useState(false)
    ...
    // sops 映射方法
    const getSopsPayload = useCallback(
      () =>
        businessProcesses.map(({ name, description, content }) => ({ name, description, content })),
      [businessProcesses]
    )
    ...
    useEffect(() => {
      if (agentDetail) {
        ...
        const vars = (agentDetail.vars || []).map((variable) => ({
          ...variable,
          varNo: variable.varNo || `var_${Date.now()}_${Math.random().toString(36).substr(2, 9)}` // 确保每个变量都有varNo
        }))
        setAgentVars(vars)

        // 初始化业务流程（sops）回显
        const initSops = (agentDetail.sops || []).map((sop, idx) => ({
          id: `process_${idx}_${Date.now()}`,
          name: sop.name || "",
          description: sop.description || "",
          content: sop.content || ""
        }))
        setBusinessProcesses(initSops)
        setIsMemoryCollapsed(vars.length === 0)
    ...
    // 业务流程折叠：有数据时自动展开，无数据时自动折叠
    useEffect(() => {
      setIsProcessCollapsed(businessProcesses.length === 0)
    }, [businessProcesses])

    // 业务流程处理函数
    const handleProcessDelete = (processId) => {
      const newProcesses = businessProcesses.filter((p) => p.id !== processId)
      ...
      onSave?.({
        ...
        knowledgeBases: knowledgeBases,
        sops: newProcesses.map(({ name, description, content }) => ({ name, description, content }))
      })
      message.success("业务流程已删除")
    }
    ...
    const handleProcessEdit = (process) => {
      ...
      setProcessPopoverVisible(true)
    }

    const handleProcessSave = () => {
      // 验证表单
      if (!processFormData.name.trim()) {
        message.error("请输入业务流程名称")
        return
      }
      if (!processFormData.description.trim()) {
        message.error("请输入业务流程描述")
        return
      }
      if (!processFormData.content.trim()) {
        message.error("请输入业务流程正文")
        return
      }

      if (editingProcessId) {
        // 编辑模式
        const updatedProcesses = businessProcesses.map((p) =>
          p.id === editingProcessId
            ? {
                ...p,
                name: processFormData.name,
                description: processFormData.description,
                content: processFormData.content
              }
            : p
        )
        setBusinessProcesses(updatedProcesses)
        // 同步保存到父组件
        onSave?.({
          pluginNos: selectedTools.filter((toolId) => toolId),
          skillNos: selectedSkills,
          knowledgeBases: knowledgeBases,
          sops: updatedProcesses.map(({ name, description, content }) => ({
            name,
            description,
            content
          }))
        })
        message.success("业务流程已更新")
      } else {
        // 新增模式
        const newProcess = {
        ...
        const newList = [...businessProcesses, newProcess]
        setBusinessProcesses(newList)
        // 同步保存到父组件
        onSave?.({
          pluginNos: selectedTools.filter((toolId) => toolId),
          skillNos: selectedSkills,
          knowledgeBases: knowledgeBases,
          sops: newList.map(({ name, description, content }) => ({ name, description, content }))
        })
        message.success("业务流程已添加")
      }

      // 重置表单
      setProcessFormData({ name: "", description: "", content: "" })
      setEditingProcessId(null)
      setProcessPopoverVisible(false)
    }

    const processFormContent = () => (
      <div className="w-[500px]">
        <div className="mb-4">
          <label className="block text-[12px] text-[#475467] font-[500] mb-2">
        ...
        <div className="mb-4">
          ...
          <Input.TextArea
            placeholder="请输入业务流程描述"
            value={processFormData.description}
            onChange={(e) =>
              setProcessFormData({ ...processFormData, description: e.target.value })
            }
            maxLength={500}
            rows={3}
          />
        ...
        <div className="mb-4">
          ...
          {/* <VariableTextArea
            value={processFormData.content}
            onChange={(e) => setProcessFormData({ ...processFormData, content: e.target.value })}
            placeholder="请输入业务流程正文"
            miniInputStyle={{
              border: "none",
              boxShadow: "none",
          ...
          /> */}
          <MDEditor
            value={processFormData.content} //processFormData.content
            onChange={(value) => {
              setProcessFormData({ ...processFormData, content: value })
            }}
            hideToolbar={false}
            preview="edit"
            height={"400px"}
        ...
        <div className="flex gap-2 justify-end">
          ...
          <Button size="small" type="primary" onClick={handleProcessSave}>
            {editingProcessId ? "更新" : "添加"}
          </Button>
        </div>
      </div>
    )
    ...
    return (
      <div className="space-y-6 -mt-[20px]">
        ...
        {type !== "voice_intelligent_mode" && (
          <div>
            <div className="text-[14px] text-[#181B25] font-[500] mb-[10px]">能力</div>
            ...
            <ToolsTable
              ...
              knowledgeBases={knowledgeBases}
              onSave={(payload) => onSave?.({ ...payload, sops: getSopsPayload() })}
              botNo={botNo}
            />
            ...
            <div className="mb-4 mt-6">
              <div className="flex items-center justify-between">
                ...
                >
                  <div className="flex items-center justify-between w-full">
                    <div className="flex items-center gap-[6px]">
                      <i
                        className={`text-[16px] text-[#475467] iconfont icon-Up transition-transform duration-300 group-hover:text-[#7f56d9] ${
                          isProcessCollapsed ? "rotate-90" : "rotate-180"
                        }`}
                      ></i>
                      <span className="text-[14px] text-[#475467] font-[500] group-hover:text-[#7f56d9]">
                        业务流程（Skills）
                ...
                {!isOnlyRead && (
                  <Popover
                    content={processFormContent()}
                    title={editingProcessId ? "编辑业务流程（Skills）" : "新增业务流程（Skills）"}
                    trigger="click"
                    open={processPopoverVisible}
                    onOpenChange={(visible) => {
                      setProcessPopoverVisible(visible)
                      if (!visible) {
                        setProcessFormData({ name: "", description: "", content: "" })
                        setEditingProcessId(null)
                      }
                  ...
                  {businessProcesses.length > 0 ? (
                    businessProcesses.map((process) => (
                      <ToolItem
                        ...
                        onDelete={!isOnlyRead && (() => handleProcessDelete(process.id))}
                        onEdit={!isOnlyRead && (() => handleProcessEdit(process))}
                      />
                    ))
                  ) : (
                    <div className="w-full">
                      <CustomEmpty description="请添加业务流程" />
                    </div>
                  )}
                </div>
              </div>
            ...
            {/* 记忆 */}
            <div className="mt-6">
              <div className="text-[14px] text-[#181B25] font-[500] mb-[10px]">变量</div>
            ...
            <div className="mt-6">
              ...
              {/* 变量部分 */}
              <div className="mb-4">
                <div className="text-[14px] text-[#7F56D9] font-[500] mb-[10px] bg-[#6841ea29] p-2 rounded-md ">
                  记忆已启用
                </div>
              </div>
            </div>
